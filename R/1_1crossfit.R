...
crossfit <- function(
  ...
  for (k in 1:fold) {
    ...
    data_test[, paste0("pseudo.ps", seq(1, q, 1))] <-
      predict(gamfit, newdata = data_test, "probs")

    for (g in seq(1, q, 1)) {
      mu_all <- estimate_mu(data_train, data_test, c.vec, k, g, q)

      pseudo1 <- (data_test$D == 1) & (data_test$X >= c.vec[g])
      data_test[pseudo1, paste0("pseudo.", g)] <-
        ifelse(!is.null(mu_all$pseudo1), mu_all$pseudo1, 0)

      pseudo0 <- (data_test$D == 0) & (data_test$X < c.vec[g])
      data_test[pseudo0, paste0("pseudo.", g)] <-
        ifelse(!is.null(mu_all$pseudo0), mu_all$pseudo0, 0)

      m1 <- (data_test$X >= c.vec[g]) &
        (data_test$X < c.vec[min(g + 1, q)]) & (data_test$D == 0)
      data_test[m1, paste0("mu.m")] <-
        ifelse(!is.null(mu_all$mu_m1), mu_all$mu_m1, 0)

      aug1 <- (data_test$X >= c.vec[g]) &
        (data_test$X < c.vec[min(g + 1, q)]) & (data_test$G == g)
      data_test[aug1, paste0("mu.aug")] <-
        ifelse(!is.null(mu_all$mu_aug1), mu_all$mu_aug1, 0)

      m0 <- (data_test$X >= c.vec[max(g - 1, 1)]) &
        (data_test$X < c.vec[g]) & (data_test$D == 1)
      data_test[m0, paste0("mu.m")] <-
        ifelse(!is.null(mu_all$mu_m0), mu_all$mu_m0, 0)

      aug0 <- (data_test$X >= c.vec[max(g - 1, 1)]) &
        (data_test$X < c.vec[g]) & (data_test$G == g)
      data_test[aug0, paste0("mu.aug")] <-
        ifelse(!is.null(mu_all$mu_aug0), mu_all$mu_aug0, 0)
    }
  ...
  for (d in c(1, 0)) {
    # storing the pseudo outcome (A.2. Step 2 Pseudo Outcome Regression)
    psd_dat <- NULL
    Lip <- matrix(0, q, q)
    ...
    for (g in seq(1, q - 1, 1)) {
      for (g.pr in seq(g + 1, q, 1)) {
        ...
        dif[g, g.pr] <- lprobust(
          temp.vc[, "psout"], temp.vc[, "X"],
          eval = eval_point, deriv = 0, p = 1, bwselect = "mse-dpi"
        )$Estimate[, 5]
        Lip[g, g.pr] <- abs(lprobust(
          temp.vc[, "psout"], temp.vc[, "X"],
          eval = eval_point, deriv = 1, p = 2, bwselect = "mse-dpi"
        )$Estimate[, 5])
      }
    }
    dif <- dif + t(-dif)
