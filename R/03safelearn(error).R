...
#     }
#
#     upper <- sapply(x.train, function(x_prime) {
#       min(1, min(dif.m + Lip * abs(x_prime - eval.main)))
#     })
#
#     lower <- sapply(x.train, function(x_prime) {
#       max(-1, max(dif.m - Lip * abs(x_prime - eval.main)))
#     })
#
...
#                        data_all$X < c.vec[g],
#                      paste0("d", 1)] <-
#               apply(cbind(eval.dat1, IND.1), 1, function(x) {
#                 sum(unlist(sapply(x[2]:x[2], function(g.temp) {
...
#                        data_all$X >= c.vec[g],
#                      paste0("d", 0)] <-
#               apply(cbind(eval.dat0, IND.0), 1, function(x) {
#                 sum(unlist(sapply((x[2] + 1):(x[2] + 1), function(g.temp) {
...
#             ###########
#             dat.temp <- data_mid %>%
#               filter(X < c.alt, X >= c.vec[g], X >= c.vec[ifelse(G == 1, 1, G - 1)], X < c.vec[G])
#
...
#
#             dat.temp <- data_mid %>%
#               filter(X >= c.alt, X < c.vec[g], X >= c.vec[G], X < c.vec[ifelse(G == q, q, G + 1)])
#
#             tempDB2 <- tryCatch(sum(with(dat.temp, eval(parse(text = paste0("pseudo.ps", g))) /
...
#       safecut_all <- full_join(safecut_all, c.all_df, by = c("group" = "group"))
#     }
#   }
#   safecut_all
# }
